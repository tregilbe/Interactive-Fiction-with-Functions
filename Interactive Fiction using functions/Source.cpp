#include <iostream>;
#include <string>;
using namespace std;

//sample functions

string GetPlayerName();
string getGoldAmount();
void DisplayIntroToUser(string Name);
string TheStoryBegins(string Name);
string TheJourney(string Name);
string Final(string Name);

int main()
{
	cout << "\n\n\t-----------Welcome to your Last Adventure-----------\t\n\n";

	// get the players name as a string
	string Name;
	Name = GetPlayerName();

	//Displaying the intro to the player
	DisplayIntroToUser(Name);

	//beginning the story	
	TheStoryBegins(Name);

	// The main event
	TheJourney(Name);

	// Close the story
	Final(Name);
}



string GetPlayerName()
{
	string Name;
	cout << "\n What is your Name? \n";
	getline(cin, Name);

	return Name;
}

void DisplayIntroToUser(string Name)
{
	cout << "\n\n\t Welcome " << Name << ", to your last Adventure. \n\n\t";

	return;
}

string TheStoryBegins(string Name)
{
	cout << " \n You awake at your camp, laying near a fire. You sit up and look around the camp and the forest around you. \n" << endl;
	cout << " \n You are accompanied by three other adventures, a bard, a rogue, and short dwarf barbarian. \n" << endl;
	cout << " \n The last thing you remember is watching your sister be absorbed by the crystal dragon you now hunt. \n" << endl;
	cout << " \n The dwarf awakes and calls to you, " << Name << " 'it's time, get up and be ready, you are our best shot at winnning this fight.' \n" << endl;
	cout << " \n As you rise and gather your gear, the others join you, everyone else is hopeful, but you can't shake this feeling in your stomach. \n" << endl;
	cout << " \n That night your sister died repeated over and over in your head. \n" << endl;

	return 0;
}

string TheJourney(string Name)
{
	cout << " \n You lead the group with the map in your hands, a walk through the woods to the nest of the crystal dragon, the last place you saw your sister. \n" << endl;
	cout << " \n As the group gets closer you start to space out, the memories of your family your head as you- '" << Name << "!' Your head jolts up at the sound of your name. \n" << endl;
	cout << " \n You hear something else, you dodge quickly to the right and barely escape a giant pink crystal shard aimed at your head. \n" << endl;
	cout << " \n Your attention is brought to the giant dragon above you as the group spreads out, 'Well if it isn't " << Name << " , I enjoyed absorbing your sister and her power, I can't wait to have your power as well!' \n" << endl;
	cout << " \n The crystal beast begins to charge his attack, your eyes fill with rage after hearing him speak her name, you yell as you transform into your griffen form. \n" << endl;
	cout << " \n Your wings spread and you feel your strength grow, your talons glowing with the enchantments of your daggers. The dragon then screeches as he seens out crystal shards, you manage to dodge them, but your group was not as fourtunate. \n" << endl;
	cout << " \n The shards fly out and pin every other member in your group, the dwarf calls out '" << Name << " it's all up to you now! Don't let her down!' \n" << endl;
	cout << " \n You keep your eyes focused on the demon as you charge, leaping into the air and spinning like a drill, aiming for the heart in the crystal. \n" << endl;
	cout << " \n As you hit your target everything goes black, and the feeling of hitting the ground knocks you out cold. \n" << endl;
	cout << " \n You open your eyes to your group around you, healing you and trying to wake you. 'You did it!' they all cheered out, the pile of crystal dust confirmed it, you had avenged her as promised. \n" << endl;
	cout << " \n You had also noticed a large pile of gold, 1000 coins to be exact. How much do you take for youself? (The rest will be given out to your group) \n" << endl;

	getGoldAmount();

	return 0;
}

string getGoldAmount()
{
	// set total gold to 1000
	int totalGold = 1000;

	// ask player how much they take
	string GoldForPlayer;
	cout << "\n How much do you take for yourself? \n";
	getline(cin, GoldForPlayer);

	return GoldForPlayer;

	// calculate whats left for rest of group
	// int goldForOthers = totalGold--GoldForPlayer;

	// then display results
	cout << "You took: " << GoldForPlayer << endl;
	// cout << "The group recieved: " << goldForOthers << endl;
}

string Final(string Name)
{
	cout << " \n As the group leaves the nest, everyone cheers and talks about celebrating at the tavern. \n" << endl;
	cout << " \n However, you decide to stay in the back of the group and stare into the broken crystal heart, remembering your last moments with your sister. \n" << endl;
	cout << " \n As the group got closer to the tavern, you cheered  up and joined the group in cheering, but once the group got to the front of the tavern they all fell silent. \n" << endl;
	cout << " \n You pushed your way to the front of the group in confusion to see what was the matter. \n" << endl;
	cout << " \n But once you got to the front, the entire town could hear the crystal heart hit the group and shatter into pieces. \n" << endl;
	cout << " \n A familiar figure stood at the door and looked at you,'It's been a while " << Name << ", how have you been sister? \n" << endl;

	return 0;
}